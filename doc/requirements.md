...
| B10 | Die API nimmt eine Anfrage in natürlicher Sprache entgegen und liefert eine per Sprachmodell (LLM) generierte Antwort darauf. Grundlage (Kontext) der Antwort sind ausschließlich die Treffer der semantischen Suche (B8) zur selben Anfrage, mit den Artist-Daten aus B9 sowie Bühne, Tag, Startzeit und Endzeit der Acts. Zu jedem Act enthält der Kontext außerdem, ob er bereits vorbei ist, gerade läuft oder noch kommt (bezogen auf die aktuelle Festival-Zeit, T3), damit die Antwort keine vergangenen Acts als bevorstehend darstellt. Die Antwort nennt nur Acts, Bühnen und Zeiten, die im Kontext stehen, und erfindet keine Festivalinformationen; lässt sich die Frage aus dem Kontext nicht beantworten, sagt die Antwort das. Nennt eine generierte Antwort nachweislich Acts, Bühnen, Tage oder Uhrzeiten, die nicht im Kontext stehen, stellt sie einen vergangenen Act nicht als vergangen dar oder behauptet sie, ein Act laufe gerade, obwohl keiner läuft, wird sie verworfen und wie eine nicht verfügbare Antwort behandelt – lieber keine Antwort als eine falsche. Liefert die Suche keine Treffer, wird kein LLM aufgerufen. Ist das LLM nicht erreichbar oder überschreitet es ein Zeitlimit, liefert die API eine erkennbare „nicht verfügbar"-Antwort statt eines generierten Texts; die Suche (B8) bleibt davon unberührt. Jede Anfrage wird einzeln beantwortet, ohne Gesprächsverlauf. |
